...
with tab1:
    ...
    if st.session_state.get("trigger_retrain", False) and len(load_log()) >= 10:
        with st.spinner("🔄 Retraining active learning model with new labels..."):
            ...
            if df_ml["label"].nunique() < 2:
                st.warning("⚠️ Not enough label variety (need both Valid and Illegal). Please label more structures.")
                ...
                if proba.shape[1] == 1:
                    # If model only saw one class
                    if model.classes_[0] == 1:   # only saw Illegal
                        st.session_state["ml_predictions"] = np.ones(len(X_full))
                    else:                        # only saw Valid
                        st.session_state["ml_predictions"] = np.zeros(len(X_full))
                else:
                    st.session_state["ml_predictions"] = proba[:, 1]
...
with tab2:
    ...
    log = load_log()
    if len(log) < 10:
        st.warning("Need at least 10 labeled samples to train the model.")
    else:
        # Use only latest decision per structure
        log_latest = log.sort_values("timestamp").drop_duplicates("structure_id", keep="last")
        df_ml = df.merge(log_latest[["structure_id", "human_decision"]], on="structure_id", how="left")
        df_ml["label"] = df_ml["human_decision"].map({"Valid": 0, "Illegal": 1, "Uncertain": np.nan})
        df_ml = df_ml.dropna(subset=["label"])

        # Safety check: need at least one sample of each class
        if df_ml["label"].nunique() < 2:
            st.warning("Not enough variety in labels (need both Valid and Illegal). Please label more structures.")
        else:
            le = LabelEncoder()
            df_ml["growth_encoded"] = le.fit_transform(df_ml["growth_type"])
            ...
            if st.button("Apply Model Predictions to All Structures"):
                ...
                # Safety: handle case where model only saw one class
                if proba.shape[1] == 1:
                    # If only one class was seen, assume probability of Illegal is 0
                    st.session_state["ml_predictions"] = np.zeros(len(X_full))
                else:
                    st.session_state["ml_predictions"] = proba[:, 1]   
